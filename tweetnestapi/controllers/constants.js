...
const detailsreturn = (media_type, data) => {
    const detailfields = {
        league:{
            team1: {
                win: data.teams[0].win,
                tower: data.teams[0].objectives.tower,
                riftHerald: data.teams[0].objectives.riftherald,
                inhibitor: data.teams[0].objectives.inhibitor,
                dragon: data.teams[0].objectives.dragon, 
                champion: data.teams[0].objectives.champion,
                baron: data.teams[0].objectives.baron, 
                participants: data.participants.filter(p => p.teamId === data.teams[0].teamId)},
            team2: {
                win: data.teams[1].win,
                tower: data.teams[1].objectives.tower,
                riftHerald: data.teams[1].objectives.riftherald,
                inhibitor: data.teams[1].objectives.inhibitor,
                dragon: data.teams[1].objectives.dragon, 
                champion: data.teams[1].objectives.champion,
                baron: data.teams[1].objectives.baron, 
                participants: data.participants.filter(p => p.teamId === data.teams[1].teamId)},
            duration: data.gameDuration,  //.info.gameDuration
            gameStart: data.gameCreation , //.info.gameCreation
            gameType: queueIds.find((q) => data.queueId === q.queueId),
        }
    };
    return detailfields[media_type]
}
const riottoken = '';
const media_types = (media_type, media_post_id) => {
    const linkreturn = {
        instagram: `https://www.instagram.com/p/${media_post_id}/?hl=en`,
        league: `https://americas.api.riotgames.com/lol/match/v5/matches/${media_post_id}`,
    };
    return linkreturn[media_type];
};
module.exports = { queueIds, detailsreturn, media_types, riottoken }